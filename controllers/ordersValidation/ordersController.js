...
export const orderDetails = async (req, res) => {
    try {
        ...
        await newOrder.save()
        const newOrderAllDetails = await Order.findById(newOrder._id).populate('userId')
        res.status(200).json(newOrderAllDetails)
    }
...
export const userOrder = async (req, res) => {
    ...
    try {
        const userOrderDetails = await Order.find({ userId }).populate('userId')
